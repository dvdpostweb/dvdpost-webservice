class PlushCustomer < ActiveRecord::Base
  ...
  has_many :plush_tokens, :foreign_key => :customer_id
  belongs_to :plush_address, :foreign_key => :customers_id, :primary_key => :customers_id, :conditions => {:address_book_id => '#{address_id}'}
  has_many :plush_actions, :foreign_key => :customerid, :class_name => 'PlushSubscription', :conditions => {:action => 7}
  has_many :plush_vod_wishlists, :foreign_key => :customer_id
  named_scope :by_custmomer, lambda {|id| {:conditions => ["customers_id > ?", id ]}}
  named_scope :shop, lambda {|id| {:conditions => ["customers_registration_step = ?", id ]}}

class StreamingProduct < ActiveRecord::Base
  ...
  def self.zen_coder
      ...
      var = <<-EOF
    {
      ...
      "input": "sftp://PS3user:ps3%40DVDPOST2014@94.139.62.130/home/PS3user/dvdpost/#{h['filename']}@#{h['imdb_id']}_Dpc_A#{h['short_lang']}_S#{h['short_sub']}_3000k.f4v",
      "region": "europe",
      "outputs":
      {
        "audio_bitrate": 64,
...
(select distinct t.imdb_id, language_id, subtitle_id, filename, quality, products_year
    ...
    results.each_hash do |h| 
      ...
      AkamaiJob.create(:imdb_id => h['imdb_id'], :language_id => h['language_id'], :subtitle_id => h['subtitle_id'])
